...
`5 类数据源` · `6 种产出格式` · `LoCoMo 1986 条 + Claude-Opus 蒸馏 8669 条（已实测）` · `离线优先·无需密钥` · `Python`
...
- **`Recipe`（配方）**：一次数据生产的完整说明书——范式、数据源、产出格式、目标条数、是否去重/课程排序/合成。
  **CLI 和未来的 skill 都先编译成同一个 `Recipe`，再交给 `pipeline.run()`**，永远只有一条执行路径。
...
| 配方 | 真实数据集 | 拉取结果 |
|------|-----------|---------|
| `hf:locomo` | `Percena/locomo-mc10` | **1986 条 QA**，每行的 `question_type` 进入 `meta.tags`（正是诊断用的能力轴） |
| `hf:jackrong-claude-opus-distill` | `Jackrong/Claude-opus-4.6-TraceInversion-9000x` | **8669 条对话**，抽样 200 条**全部携带 `<think>` 推理轨迹** |
...
- **一份合同，两个前端**：CLI 与未来的 `/agentdata` skill 都编译成同一个 `Recipe`，再调 `pipeline.run(recipe)`，
  绝不写第二条执行路径——保证「命令行能跑的，skill 也一字不差地能跑」。
...
- **GEPA 的数据观**：丰富的自然语言反馈/轨迹比堆量更重要，所以合成时给样本挂 `feedback`/`trajectory`，
  并把集合保持「小而高信号」，而不是无脑扩量。
...
flowchart TB
    ...
    subgraph CORE["pipeline.run(recipe) —— 唯一执行路径"]
        ...
        E["emit 产出<br/>sft/dpo/pretrain/chat/sharegpt/easydataset"]
    end
...
| 命令 | 作用 |
|------|------|
| `sources` | 列出数据源、HF 命名配方、`DATA_DIR` 下的本地文件 |
...
（自动扇出 + 按哈希去重 + 保留来源）。**产出格式**：`sft` `dpo` `pretrain` `chat` `sharegpt` `easydataset`。
...
├─ emit/           base(Protocol+校验) · sft/dpo/pretrain/chat/sharegpt/easydataset · convert · 工厂
...
- **LoCoMo**（[arXiv 2402.17753](https://arxiv.org/abs/2402.17753)）—— QA 类别作为诊断能力轴；persona+事件图
  作为合成重组模板。
- **GEPA**（[arXiv 2507.19457](https://arxiv.org/abs/2507.19457)）—— 丰富的自然语言反馈/轨迹胜过堆量；集合保持小而高信号。
- **Persona Hub / APIGen-MT** —— persona 驱动扩展 + 多轮智能体合成 → 重组引擎。
- **Claude-Opus 蒸馏 SFT 集** + **easy-dataset** 导出兼容。
...
| **GEPA** | 一种「丰富反馈胜过堆量」的数据观：给样本挂 feedback/trajectory，集合保持小而高信号。 |
