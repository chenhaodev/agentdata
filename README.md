...
`5 类数据源` · `6 种产出格式` · `覆盖 SFT/DPO/GRPO/CPT + 提示优化 + 多Agent + 记忆评测` · `LoCoMo 1986 条 + Claude-Opus 蒸馏 8669 条（已实测）` · `离线优先·无需密钥` · `Python`
...
| **提示/程序（不动权重）** | **GEPA** · DSPy-MIPROv2 · TextGrad · OPRO | **少量样例 + 一个指标/反馈函数** | ✗ | 小 dev 集 + 指标规范 |
